 ...
 * Date: 10 September 2024 | 24 January 2026
 * File: index.spec.js
 * Description: Test the sales report API
 * Update: Added tests for fetching sales data by customer
 */
...
// Test the sales report API - Sales by Customer
describe('Apre Sales Report API - Sales by Customer', () => {
  beforeEach(() => {
    mongo.mockClear();
  });

  // Test the sales/customers/:customer endpoint
  it('should fetch sales data for a specific customer', async () => {
    ...
    // Make a request to the endpoint
    const response = await request(app).get('/api/reports/sales/customers/CUST001');

    expect(response.status).toBe(200); // Expect a 200 status code
    // Expect response body to match expected data
    expect(response.body).toEqual([
      {
        customerId: 'CUST001',
        customerName: 'Acme Corp',
  ...
  it('should return 200 and an empty array if no sales data is found for the customer', async () => {
    // Mock the MongoDB implementation
    mongo.mockImplementation(async (callback) => {
      const db = {
        collection: jest.fn().mockReturnThis(),
    ...
    // Make a request to the endpoint
    const response = await request(app).get('/api/reports/sales/customers/UNKNOWN_CUSTOMER');
